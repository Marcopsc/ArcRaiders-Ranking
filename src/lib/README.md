...
- **Pontuação**: `src/lib/scoring.js` (`computeStats`, `tierFromVotes`). O tier de cada streamer é o tier que recebeu mais votos individuais (moda) — não a média — para que poucos votos extremos não superem streamers com muito mais votos. Em empate no número de votos, vence o tier mais alto. A ordem dentro de cada tier (`compareStatsRows`) é: quantidade de votos naquele tier → total de votos → média → nome, nessa ordem. A média (`TIER_SCORE`) continua calculada e exibida como informação complementar.
